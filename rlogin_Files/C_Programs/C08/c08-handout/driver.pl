...
# Main routine
##############
my $MAXCORR  = 100;     # Maximum correctness points
my $MAXPERF  =  20;     # Maximum performance points
my $MAXSTYLE =  60;     # Maximum style points
...
#
if ($USE_BTEST) {
    print "\n4. Compiling and running './btest -g -r 5' to determine performance score.\n";
    system("cp Zap-bits.c bits.c");
    ...
    # Run btest
    $status = system("./btest -g -r 5 > btest-Zapped.out 2>&1");
    if ($status != 0) {
	die "$0: ERROR: Zapped btest failed. $diemsg\n";
    }
}
else {
    print "\n4. Running './bddcheck/check.pl -g -r 5' to determine performance score.\n";
    system("cp Zap-bits.c bits.c");
    $status = system("./bddcheck/check.pl -g -r 5 > btest-Zapped.out 2>&1");
    if ($status != 0) {
...
#
# WDM:
#   Modified output here to render a readable table in the report to post
#   to the students.
#
#printf("%s\t%s\n", "Correctness Results", "Perf Results");
#printf("%s\t%s\t%s\t%s\t%s\t%s\n", "Points", "Rating", "Errors", 
#       "Points", "Ops", "Puzzle");
print("Correctness Results       Perf  Results   Logic\n");
print("Score   Points  Errors    Score    Ops    Descr.  Puzzle\n");
print("---------------------------------------------------------------\n");

foreach $name (sort {$puzzle_number{$a} <=> $puzzle_number{$b}} 
	       keys %puzzle_number) {
#    printf("%d\t%d\t%d\t%d\t%d\t\%s\n", 
    printf("%5d%9d%8d%9d%8d           %s\n", 
...
#
# WDM:
#   Modified output here to produce a score summary compatible with my
#   score Harvester program.
#
$idx = 1;
printf("\n>>Scores from testing<<\n");
foreach $name (sort {$puzzle_number{$a} <=> $puzzle_number{$b}} 
...
# WDM moved to driver.pl from grade-datalab.pl so that the correctness
#     and performance scores could be prefilled 10/21/2018
#
# CS@VT Note: we require the students to explain the logic of their
#     solutions, and apply a deduction of up to half of the correctness
#     credit for poor ones
#
printf("\nPart 4: Grade\n\n");
printf("Correctness: %8d /  %d\n", $total_c_rating, $MAXCORR);
printf("Performance: %8d /  %d\n", $total_p_rating, $MAXPERF);
printf("Logic:           -    /  %d\n", $MAXSTYLE);
printf("                 __________\n");
printf( "Total:                / %d\n\n", $MAXCORR+$MAXPERF);
